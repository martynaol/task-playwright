...
test.describe(`E2E tests flow`, { tag: '@smoke' }, () => {

  test.beforeEach(async ({ mainPage }) => {
    await mainPage.goto();
  });

  test(`should returns relevant search result and adds product to cart`, async ({ header, mainPage, productDetailsPage, cartPage, productName, quantity }) => {
  ...
  test(`should adds selected product, navigates to cart and proceed checkout`, async ({ mainPage, cartPage, checkoutPage, productName, quantity }) => {
  ...
  test(`should removes product from the cart`, async ({ header, mainPage, productDetailsPage, cartPage, productName, quantity }) => {
    await header.searchProductByEnter(productName);
    await mainPage.selectProductByName(productName);
    await productDetailsPage.fillInQuantity(quantity);
    await productDetailsPage.clickAddToCartBtn();
